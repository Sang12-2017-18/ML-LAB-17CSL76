...
def print_tree(node, level):
    if node.answer != "":
        print(" "*(level+2), node.answer) # Displays leaf node yes/no
        #print(" "*level,"|",end='')
        return
    if level==0:
        print(" "*level,node.attribute,"-") # Displays attribute Name
    else:
        print(" "*level,"|_",node.attribute,"-",sep='')
    #print(" "*level, "|",end='')
    for value, n in node.children:
        print(" "*(level+1),"|_", value,"-",sep='')
        #print(" "*(level+1),"|",end='')
...
''' Main program '''
dataset, features = load_csv("train_example3.csv") # Read Tennis data
node = build_tree(dataset, features) # Build decision tree
print("The decision tree for the dataset using ID3 algorithm is ")
print_tree(node,0)

test1=['Rain','Cool','Normal','Strong']
#test2=['Sunny','Mild','Normal','Strong']

print("\nThe test instance:",test1)
print("Predicted label:",end='')
classify(node,test1,features)
